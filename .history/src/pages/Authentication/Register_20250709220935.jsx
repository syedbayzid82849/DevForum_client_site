...
const Register = () => {
    const { createUserWithEP, updateProfile } = useContext(AuthContext);
    ...
    const onSubmit = (data) => {
        ...
        // Call the createUserWithEP function from AuthContext
        createUserWithEP(data.email, data.password)
            .then((userCredential) => {
                const user = userCredential.user;

                // Update displayName (and optionally photoURL)
                updateProfile(user, {
                    displayName: data.name,
                    // photoURL: data.photoURL || null  // যদি চাইলে ছবি যোগ করতে পারো
                })
                    .then(() => {
