...
const ReportedCommends = () => {
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        if (confirm.isConfirmed) {
            try {
                const res = await axiosSecure.delete(`http://localhost:3000/comments/${commentId}`, { withCredentials: true });
                if (res.data?.deletedCount > 0) {
                    await axios.delete(`http://localhost:3000/reports/by-comment/${commentId}`, { withCredentials: true });
    ...
    const handleIgnore = async (reportId) => {
        try {
            const res = await axios.delete(`http://localhost:3000/reports/${reportId}`, { withCredentials: true });
            if (res.data?.deletedCount > 0) {
                fetchReports();
